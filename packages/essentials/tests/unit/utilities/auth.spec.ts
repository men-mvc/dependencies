...
describe(`Auth Utility`, () => {
    ...
    describe(`validatePassword`, () => {
       it(`should return true when password is valid`, async () => {
           const plainTextPassword = faker.internet.password();
           const hashedPassword = await hashPassword(plainTextPassword);
           expect(await validatePassword(plainTextPassword, hashedPassword)).toBeTruthy();
       });

       it(`should return false when password is invalid`, async () => {
           const plainTextPassword = faker.internet.password();
           const hashedPassword = await hashPassword(plainTextPassword);
           expect(await validatePassword(hashedPassword, faker.internet.password())).toBeFalsy();
       });
    });
});
